...
class Wikipedia(commands.Cog):
    ...
    async def _titles(self,ctx,query:str):
        await ctx.defer()
        e=discord.Embed(title=query.title(),description='Here is some stuff I found.',colour=discord.Colour.blue())
        for i in wikipedia.search(query,results=25):
            s = wikipedia.summary(i, sentences=5)
            e.add_field(name=i, value=s,inline=False)
        e.set_footer(text='If nothing is there, it probably because wikipedia didn\'t find anything.\nThe maximum number of results is 25 due to a discord limitation.')
        e.set_author(name='Wikipedia', url='https://wikipedia.com',icon_url='https://upload.wikimedia.org/wikipedia/en/thumb/8/80/Wikipedia-logo-v2.svg/1920px-Wikipedia-logo-v2.svg.png')
